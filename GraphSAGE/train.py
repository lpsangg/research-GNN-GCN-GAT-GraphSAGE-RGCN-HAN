"""
Training Script cho GraphSAGE Model
"""
...
def train_graphsage(config):
    ...
    if config.get('model_type') == 'standard':
        model_kwargs['aggregator'] = config.get('aggregator', 'mean')
    elif config.get('model_type') == 'deep':
        model_kwargs['aggregator'] = config.get('aggregator', 'mean')
        model_kwargs['jk_mode'] = config.get('jk_mode', 'cat')
    elif config.get('model_type') == 'attention':
        model_kwargs['heads'] = config.get('heads', 4)
    elif config.get('model_type') == 'multi_aggr':
        model_kwargs['aggregators'] = config.get('aggregators', ['mean', 'max', 'min'])
    ...
    if config.get('model_type') == 'standard' or config.get('model_type') == 'deep':
        print(f"Aggregator: {config.get('aggregator', 'mean')}")
    ...
    if config.get('plot_curves', True):
        ...
        plot_training_curves(
            ...
            save_path=plot_dir / 'graphsage_training_curves.png'
        )
...
if __name__ == '__main__':
    ...
    config = {
        ...
        # Model
        'model_type': 'standard',  # 'standard', 'deep', 'attention', 'multi_aggr'
        'hidden_channels': 128,
        'num_layers': 2,
        'dropout': 0.5,
        'use_batch_norm': True,
        ...
        # GraphSAGE-specific
        'aggregator': 'mean',      # 'mean', 'max', 'min', 'lstm' (for standard/deep)
        'jk_mode': 'cat',          # 'cat', 'max', None (for deep)
        'heads': 4,                # Number of heads (for attention)
        'aggregators': ['mean', 'max', 'min'],  # List of aggregators (for multi_aggr)
