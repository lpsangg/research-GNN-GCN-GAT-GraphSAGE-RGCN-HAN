...
def train_gnn(config):
    ...
    for epoch in range(1, config.get('epochs', 100) + 1):
        ...
        if epoch % config.get('print_every', 10) == 0:
            print(f"\nEpoch {epoch}/{config.get('epochs', 100)}")
    ...
    if config.get('plot_curves', True):
        ...
        plot_training_curves(
            ...
            save_path=Path(config.get('plot_dir', 'plots')) / 'gnn_training_curves.png'
        )
...
if __name__ == '__main__':
    ...
    config = {
        ...
        # Model
        'model_type': 'basic',  # 'basic' or 'improved'
        'hidden_channels': 128,
        'num_layers': 2,
        'dropout': 0.5,
        'use_batch_norm': True,
